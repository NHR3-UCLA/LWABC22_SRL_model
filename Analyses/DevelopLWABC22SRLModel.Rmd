...
From a statistical perspective, `model 1`, `model 2`, and `model 3` have similar good predictive performance, with `model 1` being slightly better. The composite models `model 2'` and `model 3'` have poorer performance, and `model 0` is the worst. `model 0` does not consider the finite thickness of the seismogenic zone showcasing the limitations of a purely empirical model. `model 1` to `model 3'` have a break in the magnitude scaling when the width of the rupture reaches the width of the seismogenic zone allowing them to better fit the data.
...
#coefficients
df_model_pref_srl_mag_coef <- as.data.frame(t(coef(fit_model_srl1m_fxd)), row.names='model_srl1_fxd')
df_model_pref_srl_mag_coef['model_srl1_fxd','sigma'] <- sigma(fit_model_srl1m_fxd)
df_model_pref_srl_mag_coef['model_srl1_fxd','x1']    <- 1
...
#reorder model coefficients
df_model_pref_srl_mag_coef <- df_model_pref_srl_mag_coef[,c('intrcp','X1','FD','sigma')]


## Comparison of different models
fprintf("SRL Modeling:
        \tModel1 fxd.:\n\t\tLogLik: %.2f\n\t\tAIC: %.2f\n\n",
        logLik(fit_model_srl1m_fxd), AIC(fit_model_srl1m_fxd))
...
fprintf("Standard Deviations:\n\tModel 1 fxd.:\n\t\tEmpirical ratio for std of width limted to unbounded events is: %.2f\n\n",
        sig_m1_fxd_wu/sig_m1_fxd_wl)
```

```{r, srl_model_pef_mag_coeffs, echo=FALSE}
#coefficients
kable(df_model_pref_srl_mag_coef, 
      col.names = c("Intercept","$X_1$ scaling","$F_D$","$\\sigma$"),  
...
#15m seis width and dip-slip
df_predict_srl_mag$FD <- 1
df_predict_srl_mag$magnitude_lim <-  1/c2_rw * (log10(examp_seis/sin(deg2rad(examp_dip_alt))) - c1_rw )
df_predict_srl_mag$x1           <- df_predict_srl_mag$magnitude     -     pmin( log10(df_predict_srl_mag$rup_width_unbnd), log10(examp_seis/sin(deg2rad(examp_dip_alt))) )
df_predict_srl_mag$x2           <- 3/2*df_predict_srl_mag$magnitude - 2*  pmin( log10(df_predict_srl_mag$rup_width_unbnd), log10(examp_seis/sin(deg2rad(examp_dip_alt))) ) 
df_predict_srl_mag$x3           <- 3/4*df_predict_srl_mag$magnitude - 1/2*pmin( log10(df_predict_srl_mag$rup_width_unbnd), log10(examp_seis/sin(deg2rad(examp_dip_alt))) ) 
df_predict_srl_mag$x2prime      <- 3/2*df_predict_srl_mag$magnitude -     pmin( log10(df_predict_srl_mag$rup_width_unbnd), log10(examp_seis/sin(deg2rad(examp_dip_alt))) ) - 1/2*pmin(df_predict_srl_mag$magnitude,df_predict_srl_mag$magnitude_lim)
df_predict_srl_mag$x3prime      <- 3/4*df_predict_srl_mag$magnitude -     pmin( log10(df_predict_srl_mag$rup_width_unbnd), log10(examp_seis/sin(deg2rad(examp_dip_alt))) ) + 1/4*pmin(df_predict_srl_mag$magnitude,df_predict_srl_mag$magnitude_lim)
...
df_predict_srl_mag$srl2pm_d1FD    <- 10^( predict(fit_model_srl2pm,    df_predict_srl_mag) )
df_predict_srl_mag$srl3pm_d1FD    <- 10^( predict(fit_model_srl3pm,    df_predict_srl_mag) )
df_predict_srl_mag$srl1m_fxd_d1FD <- 10^( predict(fit_model_srl1m_fxd, df_predict_srl_mag) )
...
Figure 6 presents the predictions of all candidate $SRL$ models for Strike-slip events with a $15 km$ and $20 km$ width of seismogenic zone. Figure 7 presents the predictions for `model 1` and `model 1 fxd`, Figure 8 presents the predictions for `model 2` and `model 2'`, and Figure 9 presents the predictions for `model 3` and `model 3'`.
...
pl_model_srl_mag <- ggplot() + geom_point(data=df_FDHI_srl, aes(x=magnitude, event_len), color='black') +
                               ...
                               geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl2pm_d1,    color=as.factor("Model 2': D~sqrt(A)-W, FD")), size=0.8) +
                               geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl2pm_d2,    color=as.factor("Model 2': D~sqrt(A)-W, FD")), size=0.8, linetype="dashed") +
                               geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl3pm_d1,    color=as.factor("Model 3': D~sqrt(A)-L, FD")), size=0.8) +
                               geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl3pm_d2,    color=as.factor("Model 3': D~sqrt(A)-L, FD")), size=0.8, linetype="dashed") +
                               geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl1m_fxd_d1, color=as.factor("Model 1 fxd: D~sqrt(A), FD")), size=0.8) +
                               geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl1m_fxd_d2, color=as.factor("Model 1 fxd: D~sqrt(A), FD")), size=0.8, linetype="dashed") +
...
```{r fig_model_srl_mag_m1, echo=FALSE, fig.cap="Figure 7. Prediction of SRL model 1 for Strike-slip events with a 15km and 20km width of seismogenic zone"}
pl_model_srl_mag1 <- ggplot() + geom_point(data=df_FDHI_srl, aes(x=magnitude, event_len), color='black') +
                               geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl0m_d,      color=as.factor('Model 0: WC type')),                  size=0.8) +
                               geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl1m_d1,     color=as.factor('Model 1: D~sqrt(A), FD')),            size=0.8) +
                               geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl1m_d2,     color=as.factor('Model 1: D~sqrt(A), FD')),            size=0.8, linetype="dashed") +
                               geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl1m_fxd_d1,     color=as.factor('Model 1: D~sqrt(A), FD, fixed')), size=0.8) +
                               geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl1m_fxd_d2,     color=as.factor('Model 1: D~sqrt(A), FD, fixed')), size=0.8, linetype="dashed") +
                               scale_color_brewer(palette="Set1") +
                               scale_y_log10(breaks=trans_breaks("log10", function(x) 10^x), labels=trans_format("log10", math_format(10^.x))) +
                               annotation_logticks(sides = "y") + labs(x='Magnitude', y='Surface Rupture Length (km)',
                                                                       color='Model') + 
...
```{r fig_model_srl_mag_m2, echo=FALSE, fig.cap="Figure 8. Prediction of SRL model 2 for Strike-slip events with a 15km and 20km width of seismogenic zone"}
...
```{r fig_model_srl_mag_m3, echo=FALSE, fig.cap="Figure 9. Prediction of SRL model 3 for Strike-slip events with a 15km and 20km width of seismogenic zone"}
...
Figure 10 presents the predictions of `model 1 fxd` (preferred model) for the same Strike-slip events with a $15 km$ and $20 km$ width of seismogenic zone.

```{r fig_model_srl_mag_pref, echo=FALSE, fig.cap="Figure 10. Prediction of preferred SRL Model for Strike-slip events with a 15km and 20km width of seismogenic zone"}
...
pl_model_srl_mag_pref <- ggplot() + geom_point(data=df_FDHI_srl4plot, aes(x=magnitude, event_len, color=style2, size=style2, shape=bin_w)) +
                                    geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl1m_fxd_d1, linetype=as.factor(1)), size=0.8) +
                                    geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl1m_fxd_d2, linetype=as.factor(2)), size=0.8) +
...
Figure 11 presents the predictions of `model 1 fxd` for Strike-slip and Dip-slip events with a $15 km$ width of seismogenic zone.

```{r fig_model_srl_mag_pref_FD, echo=FALSE, fig.cap="Figure 11. Prediction of preferred SRL Model for Strike-slip and Dip-slip events and 15km width of seismogenic zone"}
pl_model_srl_mag_pref_FD <- ggplot() + geom_point(data=subset(df_FDHI_srl, sof>=0 & seis_depth<25), aes(x=magnitude, event_len, color=style2, size=style2)) +
                                       geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl1m_fxd_d1,   linetype=as.factor(1)), size=0.8) +
                                       geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl1m_fxd_d1FD, linetype=as.factor(2)), size=0.8) +
                                       scale_color_manual(values=color_sof, breaks=c("Normal","Reverse","Strike-Slip")) +
                                       scale_size_manual(values=c(1.4,2,2.4), breaks=c("Normal","Reverse","Strike-Slip")) +
...
Figure 12 compares the predictions of `model 0` and `model 1 fxd` for Strike-slip events with a $15 km$ and $20 km$ width of seismogenic zone. Both models have similar scaling for unbounded ruptures, but `model 1 fxd` also captures the change in scaling for width-limited ruptures leading to an overall better predictive performance.

```{r fig_model_srl_mag_wlim, echo=FALSE, fig.cap="Figure 12. Prediction of Candidate SRL Models for Strike-slip events with a 15km and 20km width of seismogenic zone. Comparison with unbounded and width-limited events"}
...
pl_model_srl_mag_wlim <- ggplot() + geom_point(data=df_FDHI_srl, aes(x=magnitude, event_len, color=flag_wlim, shape=flag_wlim), size=2) +
                                    ...
                                    geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl1m_fxd_d1,     linetype=as.factor(1)), size=0.8)   +
                                    geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl1m_fxd_d2,     linetype=as.factor(1)), size=0.8)   +
                                    scale_shape_manual(values=shape_width_flag, breaks=c(FALSE, TRUE), labels=c("Unbounded", "Width Limited"))  +
                                    scale_color_manual(values=color_width_flag, breaks=c(FALSE, TRUE), labels=c("Unbounded", "Width Limited"))  +
...
Figure 13 presents the regression residuals versus $\mathbf{M}$ for all candidate $SRL$ models.

```{r fig_models_srl_res_mag, echo=FALSE, fig.cap="Figure 13. Comparison Residuals of all candidate Models"}
pl_model_srl_res_mag <- ggplot() + geom_point(data=df_FDHI_srl, aes(x=magnitude, y=srl0m_res,     color=as.factor("Model 0: WC type"))) +
                                   ...
                                   geom_point(data=df_FDHI_srl, aes(x=magnitude, y=srl2pm_res,    color=as.factor("Model 2': D~sqrt(A)-W"))) +
                                   geom_point(data=df_FDHI_srl, aes(x=magnitude, y=srl3pm_res,    color=as.factor("Model 3': D~sqrt(A)L"))) +
                                   geom_point(data=df_FDHI_srl, aes(x=magnitude, y=srl1m_fxd_res, color=as.factor("Model 1 fxd: D~sqrt(A)"))) +
...
Figure 14 shows the regression residuals versus magnitude of `model 1 fxd` (preferred model).

```{r fig_model1_fxd_srl_res_mag, echo=FALSE, fig.cap="Figure 14. Prefered Model (Model 1, D~sqrt(A), fixed slope) Residuals versus Magnitude"}
pl_model_srl_res_mag_m1_fxd <- ggplot(data=df_FDHI_srl, aes(x=magnitude, y=srl1m_fxd_res)) + 
...
pl_model_srl_res_mag_m1_fxd
```

Figure 15 shows the `model 0` regression residuals versus magnitude.

```{r fig_model0_srl_res_mag, echo=FALSE, fig.cap="Figure 15. Linear model (Model 0, WC type) Residuals versus Magnitude"}
pl_model_srl_res_mag_m0 <- ggplot(data=df_FDHI_srl, aes(x=magnitude, y=srl0m_res)) + 
                           ...
                           annotation_logticks(sides = "y") + labs(x='Magnitude', y='Surface Rupture Length Residuals', 
                                                                   linetype='Model', color='Events', shape='Events') +
                           scale_y_continuous(breaks=c(-0.75,-0.5,-0.25,0,0.25,0.5,0.75), limits=c(-0.8,0.8)) + 
                           theme(legend.position = c(0.78, 0.15), legend.background = element_rect(size=0.1, linetype="solid", colour='black')) +
                           theme(plot.title=element_text(size=size_title),  axis.title=element_text(size=size_title), 
                                 axis.text.y=element_text(size=size_text),  axis.text.x=element_text(size=size_text),
                                 legend.title=element_text(size=size_text), legend.text=element_text(size=size_text))
...
Figure 16 shows the `model 1` regression residuals versus magnitude.

```{r fig_model1_srl_res_mag, echo=FALSE, fig.cap="Figure 16.  Candidate Model (Model 1, D~sqrt(A)) Residuals versus Magnitude"}
pl_model_srl_res_mag_m1 <- ggplot(data=df_FDHI_srl, aes(x=magnitude, y=srl1m_fxd_res)) + 
                           geom_point(aes(color=flag_wlim, shape=flag_wlim), size=2) +
                           geom_hline(yintercept=0,linetype="dashed") +
                           scale_shape_manual(values=shape_width_flag, breaks=c(FALSE, TRUE), labels=c("Unbounded", "Width Limited")) +
                           scale_color_manual(values=color_width_flag, breaks=c(FALSE, TRUE), labels=c("Unbounded", "Width Limited")) +
...
Figure 17 shows the `model 2` regression residuals versus magnitude.

```{r fig_model2_srl_res_mag, echo=FALSE, fig.cap="Figure 17. Candidate Model (Model 2, D~W) Residuals versus Magnitude"}
...
Figure 18 shows the `model 3` regression residuals versus magnitude.

```{r fig_model3_srl_res_mag, echo=FALSE, fig.cap="Figure 18. Candidate Model (Model 3, D~L) Residuals versus Magnitude"}
...
Figure 19 shows the `model 2'` regression residuals versus magnitude.

```{r fig_model2p_srl_res_mag, echo=FALSE, fig.cap="Figure 19. Candidate Model (Model 2', D~sqrt(A)-L) Residuals versus Magnitude"}
pl_model_srl_res_mag_m2p <- ggplot(data=df_FDHI_srl, aes(x=magnitude, y=srl2pm_res)) + 
                            geom_point(aes(color=flag_wlim, shape=flag_wlim), size=2) +
                            geom_hline(yintercept=0,linetype="dashed") +
                            scale_shape_manual(values=shape_width_flag, breaks=c(FALSE, TRUE), labels=c("Unbounded", "Width Limited")) +
                            scale_color_manual(values=color_width_flag, breaks=c(FALSE, TRUE), labels=c("Unbounded", "Width Limited")) +
...
pl_model_srl_res_mag_m2p
```

Figure 20 shows the `model 3'` regression residuals versus magnitude.

```{r fig_model3p_srl_res_mag, echo=FALSE, fig.cap="Figure 20. Candidate Model (Model 3', D~sqrt(A)-L) Residuals versus Magnitude"}
pl_model_srl_res_mag_m3p <- ggplot(data=df_FDHI_srl, aes(x=magnitude, y=srl3pm_res)) + 
                            geom_point(aes(color=flag_wlim, shape=flag_wlim), size=2) +
                            geom_hline(yintercept=0,linetype="dashed") +
                            scale_shape_manual(values=shape_width_flag, breaks=c(FALSE, TRUE), labels=c("Unbounded", "Width Limited")) +
                            scale_color_manual(values=color_width_flag, breaks=c(FALSE, TRUE), labels=c("Unbounded", "Width Limited")) +
...
pl_model_srl_res_mag_m3p
```

Figure 21 compares the residuals for the regression models that use the model 1 functional from with and without a fixed slope.

```{r fig_model1_cmp_srl_res_mag, echo=FALSE, fig.cap="Figure 21. Comparison Residuals of model 1 functional form models"}
pl_model1_cmp_srl_res_mag <- ggplot() + geom_point(data=df_FDHI_srl, aes(x=magnitude, y=srl1m_res,  color=as.factor('Model 1: D~sqrt(A)'))) +
                             geom_point(data=df_FDHI_srl, aes(x=magnitude, y=srl1m_fxd_res,         color=as.factor('Model 1: D~sqrt(A), fixed'))) +
                             annotation_logticks(sides = "l") + labs(x='Magnitude', y='Surface Rupture Length Residuals', color='Model') + 
                             scale_color_brewer(palette="Set1") + ylim(-.8, 0.8) +
                             theme(plot.title=element_text(size=size_title),  axis.title=element_text(size=size_title), 
                                   axis.text.y=element_text(size=size_text),  axis.text.x=element_text(size=size_text),
                                   legend.title=element_text(size=size_text), legend.text=element_text(size=size_text))
pl_model1_cmp_srl_res_mag
```

A rupture-width transition parameter $\delta$ is defined as $\delta = \log_{10}(W_{unb}) - \log_{10}(W_{lim})$; $W_{unb}$ is the theoretical rupture width for an infinite seismogenic zone estimated from the first leg of `model w1` ($\log_{10}(W_{unb}) = \beta_1 \mathbf{M} + \beta_2$). A negative value of $\delta$ corresponds to a unbounded rupture, whereas a positive corresponds to a width-limited rupture. Figure 22 shows the `model 1 fxd` regression residuals versus $\delta$. The solid black lines correspond to the rupture-width transition dependent standard deviation.

```{r fig_model1_srl_res_width, echo=FALSE, fig.cap="Figure 22 Prefered Model (Model 1, D~sqrt(A), fixed slope) Residuals versus Width transition"}
...
pl_model_srl_res_width_m1_fxd <- ggplot(data=df_FDHI_srl, aes(x=log10(rup_width_unbnd)-log10(seis_width), y=srl1m_fxd_res)) + 
                                 geom_point(aes(color=flag_wlim, shape=flag_wlim), size=2) +
                                 geom_hline(yintercept=0,linetype="dashed") +
...
show(pl_model_srl_res_width_m1_fxd)
```
...
#figures predictions
pl_model_cmp_m1_fxd <- ggplot() + geom_line(data=df_other_models,    aes(x=magnitude, y=srl_wc94_all,    linetype=as.factor('Existing'), color=as.factor('WC94 All')),    size=0.8) +
                                  ...
                                  geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl1m_fxd_d1, linetype=as.factor('Model 1 (Fixed slope)')), size=0.8) +
                                  geom_line(data=df_predict_srl_mag, aes(x=magnitude, y=srl1m_fxd_d2, linetype=as.factor('Model 1 (Fixed slope)')), size=0.8) +
                                  scale_y_log10(breaks=trans_breaks("log10", function(x) 10^x),  labels=trans_format("log10", math_format(10^.x)) ) +
                                  scale_linetype_manual(values=c("dashed", "solid"))+
...
ggsave(file.path(dir_fig,'model1_fxd_srl_mag_cmp_existing_models.png'),  plot=pl_model_cmp_m1_fxd, device='png', width=6.2, height=4)
```
